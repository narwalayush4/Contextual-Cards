...
extension FormattedTitleView {
    ...
    class ViewModel: ObservableObject {
        ...
        func splitStrings() {
            if let formattedText = card?.formattedTitle {
                ...
                for i in separatedStrings.indices {
                    if separatedStrings[i] == "{}" || separatedStrings[i] == " "{
                        ...
                        let tempEntity = Entity(text: separatedStrings[i], type: .genericText, color: nil, fontStyle: nil, fontFamily: nil, fontSize: 30)
